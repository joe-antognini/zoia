...
class TestConfig(unittest.TestCase):
    ...
    def test_save_load_config(self, mock_config_filepath):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            observed_config = zoia.backend.config.load_config()
...
class TestLibraryRoot(unittest.TestCase):
    ...
    def test_get_library_root_empty(self):
        library_root = zoia.backend.config.get_library_root()
        self.assertIsNone(library_root)

    def test_set_get_library_root(self):
        zoia.backend.config.set_library_root('foo')
        observed_library_root = zoia.backend.config.get_library_root()
